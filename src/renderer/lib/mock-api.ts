...
export const api = {
  agents: {
    ...
    // Desktop: Connect to actual tRPC PR creation
    createAgentPr: {
      useMutation: (opts?: { onSuccess?: AnyFn; onError?: AnyFn }) => {
        const mutation = trpc.git.createPR.useMutation({
          onSuccess: (data) => opts?.onSuccess?.(data),
          onError: (error) => opts?.onError?.(error),
        })
        return {
          mutate: (args: { worktreePath: string }) => mutation.mutate(args),
          mutateAsync: (args: { worktreePath: string }) => mutation.mutateAsync(args),
          isPending: mutation.isPending,
        }
      },
    },
