...
import { Brain, ChevronRight, Zap, Sparkles, Globe, Cpu, Banana as BananaIcon, Code2 } from "lucide-react"
...
import { formatCodexThinkingLabel, GEMINI_MODELS, OPENROUTER_MODELS, NANO_MODELS, BANANA_MODELS, OPENCODE_MODELS } from "../lib/models"
...
export type AgentProviderId = "claude-code" | "codex" | "gemini" | "openrouter" | "nano" | "banana" | "opencode"
...
type FlatModelItem =
  ...
  | { type: "custom" }
  | { type: "gemini"; model: { id: string; name: string; version: string } }
  | { type: "openrouter"; model: { id: string; name: string; provider: string } }
  | { type: "nano"; model: { id: string; name: string; version: string } }
  | { type: "banana"; model: { id: string; name: string; version: string } }
  | { type: "opencode"; model: { id: string; name: string; version: string } }
...
function CodexThinkingSubMenu({
  ...
  useEffect(() => {
    return () => clearTimeout(closeTimeout.current || 0)
  }, [])
