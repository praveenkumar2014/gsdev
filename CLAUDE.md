...
**gsdev Agents** - A local-first Electron desktop app for AI-powered code assistance. Users create chat sessions linked to local project folders, interact with Claude in Plan or Agent mode, and see real-time tool execution (bash, file edits, web search, etc.).
...
defaults delete dev.gsdev.agents.dev  # Dev mode
defaults delete dev.gsdev.agents      # Production
...
- Keychain profile: `gsdev-notarize`
- Create with: `xcrun notarytool store-credentials "gsdev-notarize" --apple-id YOUR_APPLE_ID --team-id YOUR_TEAM_ID`
...
1. Wait for notarization (2-5 min): `xcrun notarytool history --keychain-profile "gsdev-notarize"`
...
| `GSDEV-{version}-arm64-mac.zip` | Auto-update payload (arm64) |
| `GSDEV-{version}-mac.zip` | Auto-update payload (Intel) |
| `GSDEV-{version}-arm64.dmg` | Manual download (arm64) |
| `GSDEV-{version}.dmg` | Manual download (Intel) |

### Auto-Update Flow

1. App checks `https://cdn.gsdev.dev/releases/desktop/latest-mac.yml` on startup and when window regains focus (with 1 min cooldown)
