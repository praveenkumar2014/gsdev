// Patches the Electron.app bundle in node_modules to show "GSDEV" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName GSDEV" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName GSDEV" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> GSDEV")
  } catch (e) {
