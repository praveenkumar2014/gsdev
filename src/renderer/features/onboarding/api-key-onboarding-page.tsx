...
// Check if the key looks like a valid API key for different providers
const isValidApiKey = (key: string, provider: string) => {
  ...
  if (provider === "anthropic") {
    ...
    return trimmed.length > 0 // Ollama doesn't use API keys
  }
  return trimmed.length > 0 // Allow any key for custom providers
}
...
export function ApiKeyOnboardingPage() {
  ...
  const handleApiKeyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isValidApiKey(value, "anthropic")) {
      setTimeout(() => submitApiKey(value), 100)
    }
  }

  const handleApiKeyKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && apiKey.trim() && isValidApiKey(apiKey, "anthropic")) {
      submitApiKey(apiKey)
    }
  }
