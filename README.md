...
This guide covers the complete setup process from initial installation to deployment.
...
- **Linux**: Build essentials (for Linux builds)
...
### Step 3: Download Agent Binaries

```bash
# Download Claude binary (required for Claude Code agent)
bun run claude:download

# Download Codex binary (required for Codex agent)
...
# Add API keys for Claude, OpenAI, or other providers
```
...
Your subscription helps us maintain and improve GSDEV.
...
- **Playwright**: E2E testing framework configured in `playwright.config.ts`
- **Coverage**: Target 80% code coverage

### Writing Tests

Unit tests should be placed in `src/test/unit/` and follow the pattern `*.test.ts`:
...
E2E tests should be placed in `src/test/e2e/` and follow the pattern `*.spec.ts`:
...
```bash
# Complete release process with binary downloads
bun run release
```

This command:

1. Cleans the release directory
2. Installs dependencies
3. Downloads Claude and Codex binaries
4. Builds the application
5. Packages for macOS
6. Generates update manifest
7. Uploads release
...
This is an Electron desktop application. Deployment is done through:

1. **GitHub Releases**: Automated releases via electron-builder
2. **Auto-updater**: Built-in update mechanism for installed apps
3. **CDN Distribution**: Releases hosted at `https://cdn.gsdev.dev/releases/desktop`
...
# Upload release (requires Wrangler)
./scripts/upload-release-wrangler.sh
```

### Vercel Deployment

**Note**: This is an Electron desktop application, not a web application. Vercel is designed for web apps and static sites. For GSDEV:

- The desktop app cannot be deployed to Vercel
- Use GitHub Releases for distribution
- Use electron-builder's auto-updater for updates
- For web-based components, consider deploying separately to Vercel if needed

### Alternative Deployment Options

1. **GitHub Releases** - Recommended for Electron apps
2. **Homebrew Cask** - For macOS distribution
3. **Chocolatey** - For Windows distribution
4. **Snap Store** - For Linux distribution
5. **Flatpak** - For Linux distribution
