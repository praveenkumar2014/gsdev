...
import { BUILD_SKILLS } from "../../agents/lib/build-skills"
...
export const skillsProvider = createMentionProvider<SkillData>({
  ...
  async search(context: MentionSearchContext): Promise<MentionSearchResult<SkillData>> {
    ...
    try {
      ...
      // Add build skills
      const buildSkillsItems: MentionItem<SkillData>[] = BUILD_SKILLS.map((skill) => ({
        id: `${MENTION_PREFIXES.SKILL}${skill.id}`,
        label: skill.name,
        description: skill.description,
        icon: "skill",
        data: {
          ...
          description: skill.description,
          source: "user",
          path: `build:${skill.capability}`,
        },
        priority: 5, // Build skills have medium priority
        metadata: {
          type: "skill" as const,
        },
      }))

      items = [...buildSkillsItems, ...items]
