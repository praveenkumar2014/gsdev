...
export const ChatInputArea = memo(function ChatInputArea({
  ...
  const isVoiceAvailable = true // Force voice to always be available
  ...
  const handleVoiceMouseDown = useCallback(async () => {
    ...
    try {
      await startVoiceRecording(voiceLanguage)
    } catch (err) {
      console.error("[VoiceInput] Failed to start recording:", err)
      toast.error(err instanceof Error ? err.message : "Failed to start recording")
    }
  }, [isStreaming, isTranscribing, isVoiceRecording, startVoiceRecording, voiceLanguage])
                    ...
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        <span className="text-xs font-medium">
                          {voiceLanguage === "te-IN" ? "TE" : voiceLanguage === "sa-IN" ? "SA" : voiceLanguage === "ur-IN" ? "UR" : "EN"}
                        </span>
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => setVoiceLanguage("en-US")}>
                        English (en-US)
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => setVoiceLanguage("te-IN")}>
                        Telugu (te-IN)
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => setVoiceLanguage("sa-IN")}>
                        Sanskrit (sa-IN)
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => setVoiceLanguage("ur-IN")}>
                        Urdu (ur-IN)
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
