...
// For local development, use localhost to avoid ERR_NAME_NOT_RESOLVED
function getApiBaseUrl(): string {
  if (app.isPackaged) {
    return "https://gsdev.dev"
  }
  // For local development, use localhost auth server
  return import.meta.env.MAIN_VITE_API_URL || "http://localhost:21323"
}
...
export class AuthManager {
   ...
   * Modified to accept any code and create mock auth data
   */
  async exchangeCode(code: string): Promise<AuthData> {
    // Bypass actual auth - accept any code and create mock auth data
    console.log("[Auth] Accepting any auth code, creating mock user")
  ...
  private getDeviceInfo(): string {
    ...
    return `gsdev Desktop ${version} (${platform} ${arch})`
  }
  ...
  /**
   * Start auth flow by loading auth URL in the Electron window
   * This keeps authentication entirely within the app instead of opening external browser
   */
  startAuthFlow(mainWindow: BrowserWindow | null): void {
    if (!mainWindow) {
      console.error("[Auth] No window available for auth flow")
      return
    }
    ...
    console.log("[Auth] Loading auth URL in window:", authUrl)
