...
export type WorktreeConfigSource = "custom" | "cursor" | "gsdev" | null
...
const ONECODE_CONFIG_PATH = ".gsdev/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .gsdev/worktree.json
 */
export async function detectWorktreeConfig(
  ...
  if (await fileExists(onecodePath)) {
    ...
    if (config) {
      return { config, path: onecodePath, source: "gsdev" }
    }
  }
