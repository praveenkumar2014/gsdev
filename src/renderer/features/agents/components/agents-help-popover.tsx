...
export function AgentsHelpPopover({
  ...
  useEffect(() => {
    let cancelled = false
    window.desktopApi
      .signedFetch("https://gsdev.dev/api/changelog/desktop?per_page=3")
      .then((result) => {
        if (cancelled) return
  ...
  const handleChangelogClick = () => {
    window.desktopApi.openExternal("https://gsdev.dev/agents/changelog")
  }

  const handleReleaseClick = (version: string) => {
    window.desktopApi.openExternal(
      `https://gsdev.dev/agents/changelog#${version}`,
    )
  }
