...
function registerIpcHandlers(): void {
  ...
  const validateSender = (event: Electron.IpcMainInvokeEvent): boolean => {
    ...
    try {
      ...
      const trusted = ["gsdev.dev", "localhost", "127.0.0.1"]
...
export function createWindow(options?: { chatId?: string; subChatId?: string }): BrowserWindow {
  ...
  window.on("ready-to-show", () => {
    ...
    // Always show traffic lights on macOS for consistent window controls
    // The renderer can hide them later if needed for custom UI
    if (process.platform === "darwin") {
  ...
  // Load the renderer - bypass auth check to always load editor
  ...
  // Bypass auth check - always load the editor
  console.log("[Main] ✓ Bypassing auth check, loading editor")
