...
export class WindowsPlatformProvider extends BasePlatformProvider {
  ...
  getCliConfig(): CliConfig {
    ...
    return {
      installPath: path.join(home, ".local", "bin", "gsdev.cmd"),
      scriptName: "gsdev.cmd",
  ...
  async installCli(
    ...
    try {
      ...
      console.log("[CLI] Installed gsdev command to", installPath)
      console.log(
      ...
      return {
        success: true,
        pathHint: `To use gsdev from terminal, add to your PATH: ${installDir}`,
      }
    } catch (error: unknown) {
      const errorMessage =
  ...
  async uninstallCli(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      console.log("[CLI] Uninstalled gsdev command")
      return { success: true }
    } catch (error: unknown) {
      const errorMessage =
