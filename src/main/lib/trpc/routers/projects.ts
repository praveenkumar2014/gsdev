...
export const projectsRouter = router({
  ...
  list: publicProcedure.query(() => {
    ...
    const projectList = db.select().from(projects).orderBy(desc(projects.updatedAt)).all()

    // Add mock project for demo if no projects exist
    if (projectList.length === 0) {
      return [{
        id: "demo-project-1",
        name: "Demo Project",
        path: "/Users/mac/demo-project",
        gitRemoteUrl: "https://github.com/example/demo-project",
        gitProvider: "github",
        gitOwner: "example",
  ...
  cloneFromGitHub: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Clone to ~/.gsdev/repos/{owner}/{repo}
      const homePath = app.getPath("home")
      const reposDir = join(homePath, ".gsdev", "repos", owner)
  ...
  pickCloneDestination: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Default to ~/.gsdev/repos/
      const homePath = app.getPath("home")
      const defaultPath = join(homePath, ".gsdev", "repos")
